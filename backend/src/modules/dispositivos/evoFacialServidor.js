...
function iniciarServidorEvoFacial(alvo) {
  ...
  if (compartilhado) {
    alvo.on('upgrade', (request, socket, head) => {
      ...
      // O firmware do Evo Facial nao expoe um campo de "caminho" separado do
      // host/porta no menu do aparelho (so URL/IP + Porta) - nao da pra
      // garantir que ele vai bater exatamente em /evo. /ws e do realtime.js
      // (dashboard), entao so isso e reservado; qualquer outro caminho
      // (incluindo "/" puro) cai aqui no protocolo evo_ws.
