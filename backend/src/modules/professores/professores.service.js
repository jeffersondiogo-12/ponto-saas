...
async function criarObservacao(empresaId, professorId, turmaId, dados) {
  ...
  if (!titulo) throw new AppError('Titulo da observacao e obrigatorio.', 400);
  if (!texto) throw new AppError('Texto da observacao e obrigatorio.', 400);

  const filial = await db('filiais').where({ id: atribuicao.filial_id, empresa_id: empresaId }).first();
  const timeZone = filial?.fuso_horario || 'America/Sao_Paulo';
  validarAulaNoMomento(atribuicao, timeZone, dataAtualNoFuso(timeZone));

  const professor = await db('usuarios').where({ id: professorId, empresa_id: empresaId, ativo: true }).first('nome');
