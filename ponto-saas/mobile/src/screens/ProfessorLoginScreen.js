...
export default function ProfessorLoginScreen({ navigation }) {
  ...
  async function entrar() {
    ...
    try {
      ...
      setErro(err.message || 'Não foi possível entrar como professor.');
    } finally {
      setCarregando(false);
    }
  }
  ...
  return (
    <View style={estilos.container}>
      <Text style={estilos.marca}>Ponto<Text style={{ color: cores.brass }}>·</Text>SaaS</Text>
      <Text style={estilos.subtitulo}>Acesso do professor</Text>
      ...
      <TouchableOpacity style={estilos.link} onPress={() => navigation.navigate('Login')}>
        <Text style={estilos.linkTexto}>Voltar para aluno/responsável</Text>
      </TouchableOpacity>
    </View>
  );
}
